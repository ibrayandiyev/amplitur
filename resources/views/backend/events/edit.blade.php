...
<div class="row page-titles">
    <div class="col-md-5">
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('resources.events.edit') }}</li>
        </ol>
    </div>
    <div class="col-md-7">
        <div class="float-right">
            <a href="{{ route('backend.events.replicate', $event->id) }}" class="btn btn-sm btn-secondary ml-10">
                <i class="fa fa-copy"></i>
                {{ __('resources.events.replicate') }}
            </a>
        </div>
        <div class="float-right">
            <a href="{{ route('backend.events.create') }}" class="btn btn-sm btn-primary  ml-10">
                <i class="fa fa-plus"></i>
                {{ __('resources.events.create') }}
            </a>
        </div>
        <div class="float-right">
            <form method="GET" action="{{ route('backend.events.event_change_notification', $event->id) }}">
                <button class="btn btn-sm btn-info btn-notify-provider">
                    <i class="mdi mdi-file-pdf"> </i>{{ __('resources.bookings.event_change_notification') }}
                </button>
            </form>
        </div>
    </div>
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <form id="eventForm" method="post" action="{{ route('backend.events.update', $event->id) }}" autocomplete="off">
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $('.btn-notify-provider').on("click", function (event) {
            event.preventDefault();
            let target = event.target;
            var form = target.tagName == 'I' ? $(target).parent()[0].form : target.form;
            var title = i18next.t('question.warning.notify_providers.msg');
            ...
            swal(
                {
                    title: i18next.t('question.warning.notify_providers.title'),
                    text: title,
                    type: "warning",
