...
<div class="row">
    <div class="col-12">
        <div class="card">
            <form id="additionalsForm" method="post" action="{{ route('backend.additionals.store') }}" autocomplete="off">
                @csrf
                ...
                <div class="card-body row">
                    <div class="form-group col-md-8">
                        ...
                        <select name="package_id" class="form-control select2 m-b-10" style="width: 100%">
                            ...
                            @foreach ($packages as $package)
                                <option value="{{ $package->id }}">{{ $package->extendedName }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control" name="type">
                            <option value="{{ \App\Enums\OfferType::ALL }}" @if (old('type') == \App\Enums\OfferType::ALL) selected @endif>{{ __('resources.offers.model.types.all') }}</option>
                            <option value="{{ \App\Enums\OfferType::BUSTRIP }}" @if (old('type') == \App\Enums\OfferType::BUSTRIP) selected @endif>{{ __('resources.offers.model.types.bus-trip') }}</option>
                            <option value="{{ \App\Enums\OfferType::HOTEL }}" @if (old('type') == \App\Enums\OfferType::BUSTRIP) selected @endif>{{ __('resources.offers.model.types.hotel') }}</option>
                            <option value="{{ \App\Enums\OfferType::SHUTTLE }}" @if (old('type') == \App\Enums\OfferType::BUSTRIP) selected @endif>{{ __('resources.offers.model.types.shuttle') }}</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <select class="form-control " name="additional_group_id">
                            @foreach ($groups as $group)
                                <option value="{{ $group->id }}" @if (old('additional_group_id') == $group->id) selected @endif>{{ $group->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group col-md-2 @if($errors->has('price')) has-danger @endif">
                        ...
                        <input type="text" class="form-control input-money" name="price" value="{{ old('price') }}" />
                    </div>
