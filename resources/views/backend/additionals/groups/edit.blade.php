...
<div class="row">
    <div class="col-12">
        <div class="card">
            <form id="additionalGroupsForm" method="post" action="{{ route('backend.additionals.groups.update', $group) }}" autocomplete="off">
                @csrf
                        ...
                        <div class="form-group col-md-12 @if($errors->has('name')) has-danger @endif">
                            ...
                            <div class="tab-content br-n pn">
                                <div id="additional-group-name-pt-br" class="tab-pane active">
                                    <input type="text" class="form-control " name="name[pt-br]" value="{{ old('name.pt-br', $group->getTranslation('name', 'pt-br')) }}">
                                </div>
                                <div id="additional-group-name-en" class="tab-pane">
                                    <input type="text" class="form-control " name="name[en]" value="{{ old('name.en', $group->getTranslation('name', 'en')) }}" required>
                                </div>
                                <div id="additional-group-name-es" class="tab-pane">
                                    <input type="text" class="form-control " name="name[es]" value="{{ old('name.es', $group->getTranslation('name', 'es')) }}" required>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        <div class="form-group col-md-12">
                            ...
                            <select class="form-control" name="selection_type">
                                <option value="{{ \App\Enums\SelectionType::MULTIPLE }}" @if (old('selection_type', $group->selection_type) == \App\Enums\SelectionType::MULTIPLE) selected @endif>{{ __('resources.additionals.selection_types.multiple') }}</option>
                                <option value="{{ \App\Enums\SelectionType::SINGLE }}" @if (old('selection_type', $group->selection_type) == \App\Enums\SelectionType::SINGLE) selected @endif>{{ __('resources.additionals.selection_types.single') }}</option>
                            </select>
                        </div>
                    </div>
                </div>
