...
<div class="row page-titles">
    <div class="col-md-5">
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('resources.users.create') }}</li>
        </ol>
    </div>
    <div class="col-md-7">
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <form id="userForm" method="post" action="{{ route('backend.configs.users.store') }}" autocomplete="off">
                @csrf
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <select class="form-control @if($errors->has('status')) has-danger @endif" name="status">
                                    @foreach (App\Enums\AccessStatus::toArray() as $status)
                                        <option value="{{ $status }}" @if (old('status') == $status) selected @endif>{{ __('resources.access-status.' . $status) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
