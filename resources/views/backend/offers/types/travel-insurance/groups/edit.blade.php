...
<div class="row page-titles">
    <div class="col-md-10">
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('resources.additional-groups.edit') }}</li>
        </ol>
    </div>
</div>
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <form id="additionalGroupsForm" method="post" action="{{ route('backend.providers.companies.offers.travel-insurance.updateGroup', [$provider, $company, $offer, $additionalGroup]) }}" autocomplete="off">
                ...
                <div class="card-body">
                        ...
                        <div class="form-group col-md-6 @if($errors->has('internal_name')) has-danger @endif">
                            ...
                            <div class="tab-content br-n pn">
                                <div class="tab-pane active">
                                    <input type="text" class="form-control " name="internal_name" value="{{ old('internal_name', $additionalGroup->internal_name) }}" {{$readonly}}>
                                </div>
                            </div>
                        </div>
                        <div class="form-group col-md-6 @if($errors->has('name')) has-danger @endif">
                            ...
                            <div class="tab-content br-n pn">
                                <div id="additional-group-name-pt-br" class="tab-pane active">
                                    <input type="text" class="form-control " name="name[pt-br]" value="{{ old('name.pt-br', $additionalGroup->getTranslation('name', 'pt-br')) }}" {{$readonly}}>
                                </div>
                                <div id="additional-group-name-en" class="tab-pane">
                                    <input type="text" class="form-control " name="name[en]" value="{{ old('name.en', $additionalGroup->getTranslation('name', 'en')) }}" required {{$readonly}}>
                                </div>
                                <div id="additional-group-name-es" class="tab-pane">
                                    <input type="text" class="form-control " name="name[es]" value="{{ old('name.es', $additionalGroup->getTranslation('name', 'es')) }}" required {{$readonly}}>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        <div class="form-group col-md-12">
                            ...
                            <span class="text-danger">*</span>
                            @if($disabled)
                            <input type='hidden' name="selection_type" value='{{ $additionalGroup->selection_type }}' >
                            <select class="form-control"  disabled>
                            @else
                            <select class="form-control" name="selection_type" {{$disabled}}>
                            @endif
                                <option value="{{ \App\Enums\SelectionType::MULTIPLE }}" @if (old('selection_type', $additionalGroup->selection_type) == \App\Enums\SelectionType::MULTIPLE) selected @endif>{{ __('resources.additionals.selection_types.multiple') }}</option>
                                <option value="{{ \App\Enums\SelectionType::SINGLE }}" @if (old('selection_type', $additionalGroup->selection_type) == \App\Enums\SelectionType::SINGLE) selected @endif>{{ __('resources.additionals.selection_types.single') }}</option>
                            </select>
                        </div>
                    </div>
                </div>
