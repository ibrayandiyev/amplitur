...
<h3 class="group-entity-subhead mt-10 mb-0 p-10">
    ...
    <div class="pull-right">
        <a href="{{ route('backend.providers.companies.offers.transfer.editGroup', [$provider, $company, $offer, $additionalGroup]) }}" class="btn btn-sm btn-warning pull-right">
            <i class="fa fa-pencil"></i>
        </a>
        @if ($canChange)
        <a href="{{ route('backend.providers.companies.offers.transfer.destroyGroup', [$provider, $company, $offer, $additionalGroup]) }}" class="btn btn-danger btn-sm delete pull-right" data-toggle="tooltip" data-placement="top" title="" data-original-title="Excluir">
...
<table id="additionalItems" class="table table-bordered table-striped table-hover additionals-table pricing-tab">
    ...
    <tbody>
        ...
        <tr data-href="{{ route('backend.providers.companies.offers.transfer.updateItem', [$provider, $company, $offer, $additional]) }}">
            <td><a href="{{ route('backend.providers.companies.offers.transfer.updateItem', [$provider, $company, $offer, $additional]) }}" data-put>{{ $additional->name }}</a>
            ...
            <td class="text-center skip">
                ...
                @if ($canChange)
                <a href="{{ route('backend.providers.companies.offers.transfer.destroyItem', [$provider, $company, $offer, $additional]) }}" token="{{ csrf_token() }}" class="btn btn-danger btn-sm delete" data-toggle="tooltip" data-placement="top" title="" data-original-title="Excluir">
                    <i class="fa fa-trash"></i>
                </a>
                @endif
            </td>
