    <div class="tab-pane {{$active}}" id="tab-tb{{ $dl->id }}-{{$dl->type}}">
        <table id="trasfer" class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                @foreach ($additionalGroup->additionals as $additional)
                    <tr>
                        ...
                        <td width="30%" class=""><a href="{{ route('backend.providers.companies.offers.edit', [$offer->provider, $offer->company, $offer]) }}">{{ $additional->name }} {!! $additional->typeLabel !!}</a>
