    ...
    <tbody>
        @foreach ($additionalables as $additionalable)
            <tr class="addtitionalable-form">
                <td colspan=2 class="">{{ $additionalable->name }}</td>
            </tr>
            <tr>
                <td colspan=2 class="align-middle">
                    <div class="form-group mb-0">
                        <select class="multi-select form-control" name="additional_id[{{ $additionalable->id }}][]" style="width: 100%" multiple>
                            @php
                            $groupName = "";
                            $printFooter = 0;
                            $_sale_dates = $additionalable->getSalesDateTimestamp();
                            @endphp
                            @foreach ($additionals as $additional)
                                ...
                                @if (!empty($additionalable->additionals) && $additionalable->additionals->contains('id', $additional->id)) selected @endif>
                                {{ $additional->completeName }}</option>
                            @endforeach
                        </select>
                    </div>
                </td>
