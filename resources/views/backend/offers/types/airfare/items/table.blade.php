...
<h3 class="group-entity-subhead mt-10 mb-0 p-10">
    ...
    <div class="pull-right">
        <a href="{{ route('backend.providers.companies.offers.airfare.editGroup', [$provider, $company, $offer, $additionalGroup]) }}" class="btn btn-sm btn-warning">
            <i class="fa fa-pencil"></i>
        </a>
        @if ($canChange)
        <a href="{{ route('backend.providers.companies.offers.airfare.destroyGroup', [$provider, $company, $offer, $additionalGroup]) }}" class="btn btn-danger btn-sm delete" data-toggle="tooltip" data-placement="top" title="" data-original-title="Excluir">
...
<table id="additionalItems" class="table table-bordered table-striped table-hover additionals-table pricing-tab">
    <thead>
        <tr>
            ...
            <th class="text-center" width="8%">{{ __('resources.additionals.model.view') }}</th>
            <th class="text-center" width="11%">{{ __('resources.additionals.model.price_net') }}</th>
            <th class="text-center" width="10%">{{ __('resources.additionals.model.sale_price') }}</th>
    ...
    <tbody>
        ...
        <tr data-href="{{ route('backend.providers.companies.offers.airfare.updateItem', [$provider, $company, $offer, $additional]) }}">
            <td><a href="{{ route('backend.providers.companies.offers.airfare.updateItem', [$provider, $company, $offer, $additional]) }}" data-put>{{ $additional->name }}</a>
            ...
            <td class="text-center skip">
                ...
                @if ($canChange)
                <a href="{{ route('backend.providers.companies.offers.airfare.destroyItem', [$provider, $company, $offer, $additional]) }}" token="{{ csrf_token() }}" class="btn btn-danger btn-sm delete" data-toggle="tooltip" data-placement="top" title="" data-original-title="Excluir">
                    <i class="fa fa-trash"></i>
                </a>
                @endif
            </td>
