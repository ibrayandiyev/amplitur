@php
    $address = $bustripBoardingLocation->address;
...
<div class="row page-titles">
    <div class="col-md-12">
        <h3 class="text-themecolor">
            {{ $bustripRoute->name }} //
            {{ __('resources.bustrip-routes-boarding.edit') }}
        </h3>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">
                <a href="{{ route('backend.providers.companies.offers.bustrip.editRoute', [$provider, $company, $offer, $bustripRoute]) }}">{{ $bustripRoute->name }}</a>
                 : {{ __('resources.bustrip-routes-boarding.name') }} - {{ $bustripBoardingLocation->extendedName }}
            </li>
        </ol>
    </div>
</div>

<form id="bustripBoardingLocationForm" method="post" action="{{ route('backend.providers.companies.offers.bustrip.updateBoardingLocation', [$provider, $company, $offer, $bustripRoute, $bustripBoardingLocation]) }}" autocomplete="off">
    @csrf
    ...
    <div class="card">
        <div class="labelx label-service">
            {{ __('resources.bus-trip.info') }} - {{ __('resources.bustrip-routes-boarding.info') }}
        </div>
        ...
        <div class="card-body pricing-tab">
            <div class="row">
                <div class="form-group col-md-3 @if($errors->has('boarding_at')) has-danger @endif">
                    ...
                    <input type="text" maxlength="17" class="form-control datetimepicker" name="boarding_at" value="{{ old('boarding_at', $bustripBoardingLocation->boardingAtLocal) }}" {{$readonly}} />
                </div>
                <div class="form-group col-md-3 @if($errors->has('travel_time')) has-danger @endif">
                    ...
                    <input type="number" min="0" class="form-control" name="travel_time" value="{{ old('travel_time', $bustripBoardingLocation->travel_time ?? 60) }}" {{$readonly}} />
                </div>
                <div class="form-group col-md-2 @if($errors->has('price')) has-danger @endif">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="text" class="form-control input-money" data-bookable-price name="price" value="{{ old('price', moneyDecimal($bustripBoardingLocation->price)) }}" />
                    </div>
                </div>
                <div class="form-group col-md-2 @if($errors->has('price')) has-danger @endif">
                    <label class="form-control-label">
                    ...
                    <div class="input-group mb-2">
                        ...
                        <span class="form-control receive-span sale-price" data-bookable-receive-price name="price" value="{{ old('price', moneyDecimal($bustripBoardingLocation->price)) }}" /></span>
                    </div>
                </div>
                <div class="form-group col-md-2 @if($errors->has('is_available')) has-danger @endif">
                    <label class="form-control-label">
                        <strong>{{ __('resources.bustrip-routes-boarding.model.exhaust-availability') }}</strong>
                        <span class="text-danger">*</span>
                    </label>
                    <select class="form-control" name="is_available">
                        <option value="0" @if(old('is_available', $bustripBoardingLocation->is_available) == 0) selected @endif>{{ __('messages.no') }}</option>
                        <option value="1" @if(old('is_available', $bustripBoardingLocation->is_available) == 1) selected @endif>{{ __('messages.yes') }}</option>
                    </select>
                </div>
            </div>
            <div class="row">
            ...
            <div class="row">
                <input type="hidden" name="address[latitude]" value="{{ old('address.latitude', (float) $bustripBoardingLocation->address->latitude) }}" {{ $readonly }}>
                <input type="hidden" name="address[longitude]" value="{{ old('address.longitude', (float) $bustripBoardingLocation->address->longitude) }}" {{ $readonly }}>
        ...
        <div class="card-footer">
            ...
            <a href="{{ route('backend.providers.companies.offers.bustrip.editRoute', [$provider, $company, $offer, $bustripRoute]) }}" class="btn btn-secondary">
                <i class="fa fa-times-circle-o"></i> {{ __('resources.voltar') }}
            </a>
            @if ($canChange)
                <a href="{{ route('backend.providers.companies.offers.bustrip.destroyBoardingLocation', [$provider, $company, $offer, $bustripRoute, $bustripBoardingLocation]) }}"  class="btn btn-danger delete pull-right">
