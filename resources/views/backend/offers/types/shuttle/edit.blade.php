            ...
            <div class="col-12">
                <ul class="nav nav-tabs customtab" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link {{$active_sales_info}}" data-toggle="tab" href="#sales-info" role="tab">
                            ...
                            <span class="hidden-xs-down">{{ __('resources.shuttle.sales-info') }}</span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{$active_routes}}" data-toggle="tab" href="#routes" role="tab">
                            <span class="hidden-sm-up"><i class="ti-user"></i></span>
                            <span class="hidden-xs-down">{{ __('resources.shuttle-routes.name-plural') }}</span>
                        </a>
                    </li>
                    <li class="nav-item">
                ...
                <div class="tab-content p-4">
                    <div class="tab-pane {{$active_sales_info}}" id="sales-info" role="tab-panel">
                        ...
                        <div class="row">
                            <div class="form-group col-md-3 @if($errors->has('expires_at')) has-danger @endif">
                                <label class="form-control-label">
                                    <strong>{{ __('resources.shuttle.model.expires_at') }}</strong>
                                    <span class="text-danger">*</span>
                                </label>
                            ...
                            <div class="form-group col-md-3  @if($errors->has('currency')) has-danger @endif">
                                <label class="form-control-label">
                                    <strong>{{ __('resources.shuttle.model.sales-currency') }}</strong>
                                    <span class="text-danger">*</span>
                                </label>
                                @if($disabled)
                    ...
                    <div class="tab-pane {{$active_routes}}" id="routes" role="tab-panel">
                        @include('backend.offers.types.shuttle.routes.table')
                        <a href="{{ route('backend.providers.companies.offers.shuttle.createRoute', [$provider, $company, $offer]) }}" class="btn btn-sm btn-primary">
                            <i class="fa fa-plus"></i>
                            {{ __('resources.shuttle-routes.create') }}
                        </a>
                    </div>

                    <div class="tab-pane {{$active_additionals}}" id="additionals" role="tab-panel">
                        @include('backend.offers.types.shuttle.additionals')
                    </div>
                </div>
            </div>
        </div>
