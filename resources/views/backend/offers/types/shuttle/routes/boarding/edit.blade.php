@php
    $address = $shuttleBoardingLocation->address;
...
<div class="row page-titles">
    <div class="col-md-12">
        <h3 class="text-themecolor">
            {{ $shuttleRoute->name }} //
            {{ __('resources.shuttle-routes-boarding.edit') }}
        </h3>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">
                <a href="{{ route('backend.providers.companies.offers.shuttle.editRoute', [$provider, $company, $offer, $shuttleRoute]) }}">{{ $shuttleRoute->name }}</a>
                 : {{ __('resources.shuttle-routes-boarding.name') }} - {{ $shuttleBoardingLocation->extendedName }}
            </li>
        </ol>
    </div>
</div>

<form id="shuttleBoardingLocationForm" method="post" action="{{ route('backend.providers.companies.offers.shuttle.updateBoardingLocation', [$provider, $company, $offer, $shuttleRoute, $shuttleBoardingLocation]) }}" autocomplete="off">
    @csrf
    ...
    <div class="card">
        <div class="labelx label-service">
            {{ __('resources.shuttle.info') }} - {{ __('resources.shuttle-routes-boarding.info') }}
        </div>

        <div class="card-body pricing-tab">
            <div class="row">
                <div class="form-group col-md-4 @if($errors->has('address.complement')) has-danger @endif">
                    <label class="form-control-label">
                        <strong>{{ __('resources.shuttle-routes-boarding.model.complement') }}</strong>
                        <span class="text-danger">*</span>
                    </label>
                    <input type="text" class="form-control" name="address[complement]" value="{{ old('address.complement', $address->complement) }}" {{$readonly}} >
                </div>
                <div class="form-group col-md-2 @if($errors->has('boarding_at')) has-danger @endif">
                    <label class="form-control-label">
                        <strong>{{ __('resources.shuttle-routes-boarding.model.boarding_at') }}</strong>
                        <span class="text-danger">*</span>
                    </label>
                    <input type="text" maxlength="17" class="form-control datetimepicker" name="boarding_at" value="{{ old('boarding_at', $shuttleBoardingLocation->boardingAtLocal) }}"  {{$readonly}} />
                </div>
                <div class="form-group col-md-2 @if($errors->has('price')) has-danger @endif">
                    <label class="form-control-label">
                        <strong>{{ __('resources.shuttle-routes-boarding.model.net_sale') }}</strong>
                        <span class="text-danger">*</span>
                    </label>
                    <div class="input-group mb-3">
                        ...
                        <input type="text" class="form-control input-money" data-bookable-price name="price" value="{{ old('price', moneyDecimal($shuttleBoardingLocation->price)) }}" />
                    </div>
                </div>
                <div class="form-group col-md-2 @if($errors->has('price')) has-danger @endif">
                    <label class="form-control-label">
                        <strong>{{ __('resources.shuttle-routes-boarding.model.sale_price') }}</strong>
                        <span class="text-danger">*</span>
                    </label>
                    <div class="input-group mb-2">
                        ...
                        <span class="form-control receive-span sale-price" data-bookable-receive-price name="price" value="{{ old('price', $shuttleBoardingLocation->price) }}" /></span>
                    </div>
                </div>
                <div class="form-group col-md-2 @if($errors->has('is_available')) has-danger @endif">
                    <label class="form-control-label">
                        <strong>{{ __('resources.shuttle-routes-boarding.model.exhaust-availability') }}</strong>
                        <span class="text-danger">*</span>
                    </label>
                    <select class="form-control" name="is_available">
                        <option value="0" @if(old('is_available', $shuttleBoardingLocation->is_available) == 0) selected @endif>{{ __('messages.no') }}</option>
                        <option value="1" @if(old('is_available', $shuttleBoardingLocation->is_available) == 1) selected @endif>{{ __('messages.yes') }}</option>
                    </select>
                </div>
            </div>
            <div class="row">
                ...
                <div class="form-group col-md-3 @if($errors->has('travel_time')) has-danger @endif">
                    <label class="form-control-label">
                        <strong>{{ __('resources.shuttle-routes-boarding.model.travel_time') }}</strong>
                        <span class="text-danger">*</span>
                    </label>
                    <input type="number" min="0" class="form-control" name="travel_time" value="{{ old('travel_time', $shuttleBoardingLocation->travel_time ?? 60) }}" {{ $readonly }}>
                </div>
            </div>
            <div class="row">
                <input type="hidden" name="address[latitude]" value="{{ old('address.latitude', (float) $shuttleBoardingLocation->address->latitude) }}" {{ $readonly }}>
                <input type="hidden" name="address[longitude]" value="{{ old('address.longitude', (float) $shuttleBoardingLocation->address->longitude) }}" {{ $readonly }}>
        ...
        <div class="card-footer">
            ...
            <a href="{{ route('backend.providers.companies.offers.shuttle.editRoute', [$provider, $company, $offer, $shuttleRoute]) }}" class="btn btn-secondary">
                <i class="fa fa-times-circle-o"></i> {{ __('resources.voltar') }}
            </a>
            @if ($canChange)
                <a href="{{ route('backend.providers.companies.offers.shuttle.destroyBoardingLocation', [$provider, $company, $offer, $shuttleRoute, $shuttleBoardingLocation]) }}"  class="btn btn-danger delete pull-right">
