        ...
        @foreach ($productsBt as $product)
            <div class="servico-principal-data grupo-bus-trip-{{$keyRoute}}">
                ...
                <div class="form__checkbox form__checkbox--radio servico-principal servico-principal__grupo-{{ $product['type'] }} servico-principal__fatorpass-{{ $product['capacity'] }} servico--{{ $boardingLocation->getStockStatusClass() }}" id="servico-principal-{{ $key }}">
                    <input class="rd-servico-principal rd-servico-principal-produto" type="radio" name="servprin" value="{{ $product['offer_id'] }}" data-product="{{ $product['id'] }}" data-valor="{{ moneyFloat($product['price'], currency(), $product['offer']->currency) }}" data-cotacao="1.00" data-moeda="{{ currency()->code }}" data-fatorpass="{{ $product['capacity'] }}"
