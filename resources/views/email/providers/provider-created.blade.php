                ...
                <table class="table">
                    <tr>
                        <td class="td_padding">
                            ...
                            <p>{{__('mail.provider.registry.reg_thanks')}}</p>

                            <p><b>{!!__('mail.provider.registry.reg_attention')!!}</b> </p>
                            <p> {{__('mail.provider.registry.valid_link')}}</p>
                            <CENTER>
                                <a href="{{ route('backend.providers.validateToken', $provider->validation_token) }}" class="link">{{__('mail.provider.registry.link_valid')}}</a><br><br>

                                <span style="font-size: 12px"> <strong><i>{{__('mail.provider.registry.link_valid_type')}}</i></strong></span><br>
                                <span style="font-size: 10px"> <i>{{ route('backend.providers.validateToken', $provider->validation_token) }}</i></span>
                            </CENTER>

                            <P>{{__('mail.provider.registry.client_msg')}}</P>

                            <p>
                                <B>{{__('mail.provider.registry.name')}}: </B><I>{{ $provider->name}}</I><br>
                                <B>{{__('mail.provider.registry.email')}}: </B><I>{{ $provider->email}}</i><br>
                                <B>{{__('mail.provider.registry.login')}}: </B><i>{{ $provider->username}}</i><br>
