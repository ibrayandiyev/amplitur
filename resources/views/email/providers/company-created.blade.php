                ...
                <table class="table">
                    <tr>
                        <td class="td_padding">
                            ...
                            <p>{{__('mail.provider.company.reg_thanks')}}</p>

                            <p><b>{!!__('mail.provider.company.reg_attention')!!}</b> </p>

                            <P>{{__('mail.provider.company.client_msg')}}</P>

                            <p>
                                <B>{{__('mail.provider.company.name')}}: </B><I>{{ $company->legal_name}}</I><br>
                                <B>{{__('mail.provider.company.name_fantasy')}}: </B><I>{{ $company->company_name}}</i><br>
                                @php
                                    $city = $address->city();
                                    if(is_object($city)){
                                        $city = $city->name;
                                ...
                                @endphp
                                <B>{{__('mail.provider.company.address')}}: </B><i>{{ $address->address}} - {{ $address->number}}, {{$city}} ,{{ $address->country()->name}}</i><br>
                            </p>

                            <p>

                                <a href="{{ route('backend.index') }}">
                                    <span><b>{{__('mail.provider.company.clique_aqui')}}</b></span>
                                </a>{{__('mail.provider.company.login_msg')}}

                            </p>

                            <p>{{__('mail.provider.company.doubt_msg')}}</p>
